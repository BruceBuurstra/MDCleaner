...
#Function to drop the columns having x% or more mising values.
drop_columns <- function(x) {
  # dropping the columns having x% or more 'NA' values using discard
  ratings_wide <- ratings_wide %>% purrr::discard(~sum(is.na(.x))/length(.x) >=x)
  #ratings <- ratings %>% purrr::discard(sum(is.na(.x))/length(.x)* 100 <=x)
...
run_nmf_imputation <- function(z){
  ...
  ratings_nmf_imputed <- matrix(unlist(nmf_mult_round), ncol = ncol(ratings_wide_m), byrow = FALSE)
...
find_optimal_nmf_kmeans_clusters <- function(){
  movies <- t(nmf@h)
  return(fviz_nbclust(movies, kmeans, method = "silhouette", k.max=20))
}

find_optimal_nmf_kmeans_clusters()
...
run_nmf_movies_clustering <- function(w){
  ...
  plot1 <- fviz_cluster(kmean, data = movies, geom = c("point"),ellipse.type = "euclid")
...
run_knn_imputation <- function(n){
  ...
  for (i in 1:ncol(ratings_knn_imputed)){
    ...
    for (j in 1:nrow(ratings_knn_imputed))
    {
      if (ratings_knn_imputed[j,i]==0)
      {
        ...
        {
          avg_knn_values <- sum(knn_non_zero_values)/length(knn_non_zero_values)
  ...
  ratings_knn_imputed <- matrix(unlist(knn_round), ncol = ncol(ratings_wide_m), byrow = FALSE)
...
run_knn_movies_clustering <- function(v){
  ...
  plot1 <- fviz_cluster(knn_kmeans, data = knn_movies, geom = c("point"),ellipse.type = "euclid")
